...
export default function PortfolioGallery({ items }: { items: PortfolioGalleryItem[] }) {
  ...
  const activeEntry = active ? items[active.entryIndex] : null;
  ...
  function goToImage(nextIndex: number) {
    setActive((current) => {
      ...
      const images = getEntryImages(items[current.entryIndex]);
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === "ArrowLeft") {
        setActive((current) => {
          ...
          return {
            ...current,
            imageIndex: ((current.imageIndex - 1) % images.length + images.length) % images.length,
          };
        });
      }
  ...
  return (
    <>
      ...
      {typeof window !== "undefined" && activeEntry
        ? createPortal(
        <div
          role="dialog"
          aria-modal="true"
        ...
        >
          ...
          >
            ...
            >
              <img
                src={activeImages[active.imageIndex]}
                alt={`${activeEntry.title} ${active.imageIndex + 1}`}
                style={{
                  width: "100%",
            ...
            {activeImages.length > 1 ? (
              ...
              >
                {activeImages.map((img, imageIndex) => {
                  const selected = imageIndex === active.imageIndex;
